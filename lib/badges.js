...
module.exports = async function badges (server, options, next) {
  ...
  const getAllBadges = async (colorB, text) => ({
    ...
    'for-the-badge': await ghBadges(defaultOpts('for-the-badge', colorB, text))
  })
  ...
  const getEtags = state => ({
    ...
    'for-the-badge': createHash(state['for-the-badge'])
  })
  ...
  async function handleBadges (request, reply) {
    ...
    const style = ['flat', 'flat-square', 'plastic', 'for-the-badge'].includes(request.query.style) ? request.query.style : 'flat'
